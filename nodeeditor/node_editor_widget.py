...
class NodeEditorWidget(QWidget):
    ...
    def getUserFriendlyFilename(self):
        name = os.path.basename(self.filename) if self.isFilenameSet() else "Новый график"
    ...
    def addNodes(self):
        node1 = Node(self.scene, "Вещь", inputs=[0,0,0], outputs=[1])
        node2 = Node(self.scene, "Input", inputs=[3,3,3], outputs=[1])
        node3 = Node(self.scene, "Add", inputs=[2,2,2], outputs=[1])
